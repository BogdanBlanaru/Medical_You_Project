...
public class PatientProfile {
    ...
    // JSON Arrays stored as TEXT (will be parsed manually)
    ...
    public void generateMedicalId() {
        if (this.medicalId == null) {
            // Format: MED-XXXXXXXX (8 random alphanumeric characters)
            this.medicalId = "MED-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase();
        }
    }
    ...
    // JSON Helper methods for medications
    public List<Medication> getMedicationsList() {
        if (currentMedications == null || currentMedications.isEmpty()) {
            return new ArrayList<>();
        }
        try {
            return objectMapper.readValue(currentMedications, new TypeReference<List<Medication>>() {});
        } catch (JsonProcessingException e) {
            return new ArrayList<>();
        }
    }

    public void setMedicationsList(List<Medication> list) {
        try {
    ...
    // Medication class (no longer @Embeddable, just a POJO for JSON)
    @Data
    @NoArgsConstructor
    @AllArgsConstructor
    @Builder
